  ...
  <div class="fio-detail">
    ...
    <div v-else-if="error" class="error-container card">
      ...
      <div class="error-actions">
        <button @click="fetchFioData" class="btn btn-primary">重试</button>
        <router-link to="/fio" class="btn btn-secondary">返回列表</router-link>
      </div>
    </div>
    ...
    <template v-else>
      <div class="card">
        <div class="card-header">
          <h2>{{ fio.metadata.name }}</h2>
          <span class="status-badge" :class="getStatusClass(fio.status?.status)">
        ...
        <div class="card-body">
          <div class="info-grid">
            <div class="info-item">
              <div class="info-label">测试路径</div>
              <div class="info-value">{{ fio.spec.filePath }}</div>
            </div>
            <div class="info-item">
              <div class="info-label">测试类型</div>
              <div class="info-value">{{ getReadWriteMode(fio.spec.readWrite) }}</div>
            </div>
            <div class="info-item">
              <div class="info-label">文件大小</div>
              <div class="info-value">{{ fio.spec.size }}</div>
            </div>
            <div class="info-item">
              <div class="info-label">块大小</div>
              <div class="info-value">{{ fio.spec.blockSize }}</div>
            </div>
            <div class="info-item">
              <div class="info-label">I/O深度</div>
              <div class="info-value">{{ fio.spec.ioDepth || 1 }}</div>
            </div>
            <div class="info-item">
              <div class="info-label">最后执行时间</div>
              <div class="info-value">{{ formatDate(fio.metadata.creationTimestamp) }}</div>
            </div>
          </div>
        ...
        <div class="card-body">
          <div class="summary-grid">
            <div class="summary-item">
              ...
              <div class="summary-value">{{ fio.status.queryCount || 0 }}</div>
            </div>

            <div class="summary-item">
              <div class="summary-label">成功次数</div>
              <div class="summary-value">{{ fio.status.successCount || 0 }}</div>
            </div>

            <div class="summary-item">
              <div class="summary-label">失败次数</div>
              <div class="summary-value">{{ fio.status.failureCount || 0 }}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="card">
        ...
        <div class="card-body">
          <pre v-if="fio.status.lastResult" class="output-pre">{{ fio.status.lastResult }}</pre>
      ...
      <div v-if="fio.status.testReportName" class="card">
        <div class="card-header">
          <h3>测试报告</h3>
          <button
        ...
        <div v-else-if="testReport">
          <h4 class="mb-4">测试摘要</h4>
              <div class="summary-grid">
                <div class="summary-item">
                  <div class="summary-label">读取带宽</div>
                  <div class="summary-value">{{ fio.status.stats.readBW || '-' }}</div>
                </div>
                <div class="summary-item">
                  <div class="summary-label">写入带宽</div>
                  <div class="summary-value">{{ fio.status.stats.writeBW || '-' }}</div>
                </div>
                <div class="summary-item">
                  <div class="summary-label">读取IOPS</div>
                  <div class="summary-value">{{ fio.status.stats.readIOPS || '-' }}</div>
                </div>
                <div class="summary-item">
                  <div class="summary-label">写入IOPS</div>
                  <div class="summary-value">{{ fio.status.stats.writeIOPS || '-' }}</div>
                </div>
                <div class="summary-item">
                  <div class="summary-label">读取延迟</div>
                  <div class="summary-value">{{ fio.status.stats.readLatency || '-' }}</div>
                </div>
                <div class="summary-item">
                  <div class="summary-label">写入延迟</div>
                  <div class="summary-value">{{ fio.status.stats.writeLatency || '-' }}</div>
                </div>
              </div>
...
export default {
  setup() {
    ...
    const fioName = computed(() => route.params.name)
    ...
    const isRunning = computed(() => {
      if (!fio.value || !fio.value.status) return false

      const status = (fio.value.status.status || '').toLowerCase()
    ...
    const configDetails = computed(() => {
      ...
      const details = { ...fio.value.spec }
      return details
    })

    // 获取Fio详情
    const fetchFioData = async () => {
      loading.value = true
      error.value = null

      try {
        const response = await fioApi.getFio(fioName.value)
        fio.value = response
        ...
        error.value = '获取IO性能测试详情失败: ' + (err.message || '未知错误')
        console.error(err)
      } finally {
        loading.value = false
      }
    ...
    const fetchTestReport = async () => {
      if (!fio.value || !fio.value.status.testReportName) return

      loadingTestReport.value = true
      testReportError.value = null

      try {
        const response = await fioApi.getFioTestReport(fio.value.status.testReportName)
    ...
    const deleteFio = async () => {
      ...
      try {
        await fioApi.deleteFio(fioName.value)
        router.push('/fio')
      } catch (err) {
